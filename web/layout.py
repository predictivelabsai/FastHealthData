"""FastHealthData 3-pane layout: top bar · left nav · center · right AI rail.

Server-rendered FastHTML + HTMX; one block of vanilla JS for the streaming chat.
Adapted from the shared fasthtml-oss-migrations shell (see FastHelpdesk).
"""
...
# FastHealthData palette — clinical indigo, teal accents, amber/rose signals.
LAYOUT_CSS = """
:root{
  --bg:#f4f6fb; --surface:#ffffff; --surface-2:#eef1f8; --border:#dde3ef; --text:#141a2e;
  --text-dim:#4a5270; --text-mute:#8891a8; --accent:#4f46e5; --accent-hover:#4338ca;
  --accent-light:#e5e3fb; --ok:#0e9f6e; --ok-light:#def7ec; --warn:#c27803; --warn-light:#fdf6b2;
  --breach:#e02424; --breach-light:#fde8e8; --teal:#0694a2; --teal-light:#d5f5f6;
}
...
.app{display:grid;grid-template-columns:238px 1fr var(--rail,340px);grid-template-rows:52px 1fr;
...
.kpi.breach::after{background:var(--breach);} .kpi.warn::after{background:var(--warn);} .kpi.ok::after{background:var(--ok);} .kpi.teal::after{background:var(--teal);}
...
.pill.registered{background:#e2e8f0;color:#475569;} .pill.dataintake{background:var(--teal-light);color:#036672;}
.pill.analysis{background:var(--accent-light);color:var(--accent-hover);} .pill.outputs{background:var(--ok-light);color:#046c4e;} .pill.closed{background:#e5e7eb;color:#6b7280;}
.pill.pending{background:var(--warn-light);color:#8e4b10;} .pill.approved{background:var(--ok-light);color:#046c4e;} .pill.rejected{background:var(--breach-light);color:#9b1c1c;}
.pill.anonymised{background:var(--ok-light);color:#046c4e;} .pill.pseudonymised{background:var(--teal-light);color:#036672;} .pill.identifiable{background:var(--breach-light);color:#9b1c1c;}
.pill.omopcdm{background:var(--accent-light);color:var(--accent-hover);} .pill.hl7fhir{background:#dbeafe;color:#1e429f;} .pill.openehr{background:var(--teal-light);color:#036672;} .pill.custom{background:#e5e7eb;color:#6b7280;}
.pill.researcher{background:var(--accent-light);color:var(--accent-hover);} .pill.datasteward{background:var(--teal-light);color:#036672;} .pill.dpo{background:var(--warn-light);color:#8e4b10;} .pill.admin{background:#e5e7eb;color:#374151;}
.pill.ok2{background:var(--ok-light);color:#046c4e;} .pill.neutral{background:#e5e7eb;color:#475569;}
.tag{display:inline-block;padding:1px 7px;border-radius:5px;font-size:10.5px;font-weight:600;background:var(--breach-light);color:#9b1c1c;}
...
.btn.danger{border-color:var(--breach);color:var(--breach);} .btn.danger:hover{background:var(--breach);color:#fff;}
.toolbar{display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap;}
.toolbar input[type=search],.toolbar input[type=text]{padding:8px 12px;border:1px solid var(--border);border-radius:8px;font-size:13px;min-width:240px;}
.mini-select{padding:3px 7px;border:1px solid var(--border);border-radius:6px;font-size:12px;background:var(--surface);max-width:200px;}
.field{width:100%;padding:9px 11px;border:1px solid var(--border);border-radius:8px;font-size:13px;font-family:inherit;margin-bottom:8px;}
.field:focus{border-color:var(--accent);outline:none;box-shadow:0 0 0 3px var(--accent-light);}
.plot{width:100%;min-height:260px;}
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:var(--surface-2);padding:2px 7px;border-radius:5px;font-size:12.5px;}
...
.callout.warn{background:var(--warn-light);border-left-color:var(--warn);color:#8e4b10;}
.login-wrap{height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#e9e7fb 0%,#d5f5f6 100%);}
...
NAV_ITEMS = [
    ...
    ("RESEARCH", [("projects", "Projects", "🧪", "/projects"),
                  ("catalog", "Metadata Catalog", "🗂️", "/catalog")]),
    ("GOVERNANCE", [("access", "Access Governance", "🔐", "/access"),
                    ("pseudonymise", "Pseudonymisation", "🕵️", "/pseudonymise"),
                    ("audit", "Audit Log", "📜", "/audit")]),
    ("INSIGHT", [("analytics", "Analytics", "📈", "/analytics")]),
    ("HELP", [("guide", "User Guide", "📖", "/guide")]),
]
SAMPLE_QUESTIONS = [
    "Which access requests are pending?",
...
def topbar(env, user_email):
    ...
    return Div(Div(Span(cls="brand-dot"), Span("Fast", style="font-weight:800;"),
                   Span("HealthData", style="color:var(--accent);font-weight:700;letter-spacing:.5px;"), cls="brand"),
               right, cls="topbar")
...
def right_pane_chat(thread_id):
    return Div(
        Div(H3("AI Assistant"),
            ...
            cls="right-header"),
        Div(Div(P("Ask about projects, datasets, access requests or the synthetic cohort — or tap a question below.",
            ...
            Form(Input(type="hidden", name="thread_id", value=thread_id, id="thread-id"),
                 Div(Input(type="text", name="message", id="chat-input",
                           placeholder="Ask a question or /projects /access /cohort /help …", autocomplete="off"),
